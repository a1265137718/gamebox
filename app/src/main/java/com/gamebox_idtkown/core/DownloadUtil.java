...
public class DownloadUtil {
    ...
    public static void downloadApk(DownloadInfo downloadInfo, FileDownloadListener fileDownloadListener, int times){
        ...
        downloadInfo.downloadTask = task;
        task.setSyncCallback(true).setAutoRetryTimes(AutoRetryTimes).setMinIntervalUpdateSpeed(MinIntervalUpdateSpeed).setCallbackProgressMinInterval(CallbackProgressMinInterval).setPath(PathUtil
                .getApkPath(downloadInfo
                        .getName()), false).setCallbackProgressTimes(CallbackProgressTimes).setListener
                (fileDownloadListener);
    }
